...
def plot_classification_report(classification_report, title='Classification report ', cmap='RdBu', from_conll_json=False):
    ...
    if from_conll_json:
        ...
        for line in lines[2 : (len(lines) - 1)]:
            #print(line.strip().replace('avg / total', 'micro-avg'))
            t = line.strip().replace('avg / total', 'micro-avg')
            t = t.replace('micro avg', 'micro-avg')
            t = t.replace('macro avg', 'macro-avg')
            t = t.replace('weighted avg', 'weighted-avg')
            t = t.split()
